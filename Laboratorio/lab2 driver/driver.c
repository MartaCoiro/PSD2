...
int main(void)
{
	...
	int arr_input[N]; //arr_input   -> conterrà una linea del file ..input.xt
	...
	int test; // è usata per esprimere il risultato del testing:  0 -> fallimento 1- > successo
	int i, pos;
	...
	for(i=1; fgets(line, N, fp_input)!=NULL; i++){
		...
		n_input=input_array_str (arr_input, line, &pos);
